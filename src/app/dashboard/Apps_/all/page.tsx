...
const AllCustomDataPage = () => {
  const [allData, setAllData] = useState<appDatatype[]>([]);

  useEffect(() => {
    const fetchApps = async () => {
      try {
        const res = await fetch("/api/dashboard/customApp/getAllUserApp");
        console.log(res);
      ...
      <div className="h-140 border border-gray-200 p-6 pb-15  bg-white flex flex-col gap-5 rounded-lg overflow-y-auto">
        {allData.map((data, index) => (
          <div key={index} className="flex flex-col gap-5">
            <Link
              href="#"
              className="flex items-center group group-hover:bg-purple-50 cursor-pointer flex-col sm:flex-row justify-between"
            >
              <div className="flex gap-4 items-center">
                ...
                height={32}
                  ...
                  alt={data.appName}
                />
                <h2 className="font-semibold text-gray-800">{data.appName}</h2>
              </div>
...
export default AllCustomDataPage;
